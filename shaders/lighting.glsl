...
float calculateShadowPCFPoisson(Light light, vec4 lightSpacePosition, sampler2D shadowMapTex)
{
    ...
    vec2 rotation = vec2(random(lightSpacePosition.xyz, lightSpacePosition.x), random(lightSpacePosition.xyz, lightSpacePosition.y));
    ...
    for (int i = 0; i <= samples; i++)
    {
        ...
        shadowFactor += calculateShadow(light, shadowMapTex, projCoords + vec3(
            poissonOffset.x * scaleDelta, poissonOffset.y * scaleDelta, 0));
    }
